...
class Command(BaseCommand):
    help = 'Seed default data for all tables'

    def handle(self, *args, **kwargs):
        # Tạo tài khoản admin
        if not User.objects.filter(username='admin').exists():
            User.objects.create_superuser(username='admin', password='admin', email='admin@example.com')
        # Tạo tài khoản sinh viên MSSV
        if not User.objects.filter(username='21078321').exists():
            User.objects.create_user(username='21078321', password='123')
